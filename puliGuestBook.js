...
$.puliGuestBook = function (config) {
    ...
    if ($('head link[href="'+css+'"]').length == 0)
        $("<link type='text/css' rel='stylesheet' href='"+css+"' /> ").appendTo($('head'));
    
    var container = $(getParam('container', '#puliGuestBook'));
    if (container.length == 0)
    {
        document.write('<div id="pulipuli_guestbook">'+'Loading...'+'</div>');
        
        container = $('#pulipuli_guestbook');
    }
